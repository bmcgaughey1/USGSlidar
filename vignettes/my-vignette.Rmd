...
specific years. Other USGS data collection contain this information. I have developed R code that
matches index files from the data hosted on the rockyftp server with the projects in the Entwine
collection to produce a new index file that can be used with the **USGSlidar** package. However,
this index is not currently hosted in a location accessible to others. I will make this index and the 
R code used to create it available soon.

The example point is located within the 2014 Glacier Peak lidar acquisition in Washington state, USA.

**This example will download the [Entwine index](https://raw.githubusercontent.com/hobu/usgs-lidar/master/boundaries/resources.geojson) produced by Howard Butler (4.9Mb) onto the current working directory.**
